...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 23-04-2015 a las 22:56:50
...
FROM tblCredito cr inner join tblCliente cl on (cr.`idCliente`=cl.`idCliente`) where cl.`numeroDocumento` = numeroDocumen; 
END$$
...
ALTER TABLE `tblcredito`
  ADD CONSTRAINT `tblcredito_ibfk_1` FOREIGN KEY (`idCliente`) REFERENCES `tblcliente` (`idCliente`);
