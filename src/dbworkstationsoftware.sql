...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 08-05-2015 a las 22:26:20
...
BEGIN
	UPDATE tblCredito SET  estadoCredito = estadoCredi 
        WHERE idCredito = idCredi;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `spConsultarCreditoByIdCredito`(   
    in idCredi  int
)
BEGIN

SELECT idCredito, idCategoriaCredito, documentoUsuario, fechaInicio, saldoInicial, saldoActual, estadoCredito 
FROM tblCredito
WHERE idCredito = idCredi;
...
BEGIN
	declare msg varchar(40);    
	if (exists(select documentoUsuario 
        from tblCredito 
        where documentoUsuario=documentoUsuar and estadoCredito>0 
        and idCategoriaCredito=1 and idCategoriaCredito=2)) then
		set msg="Este usuario ya tiene un credito activo.";
		select msg as Respuesta;
       	else
		insert into tblCredito (idCategoriaCredito,documentoUsuario,fechaInicio,saldoInicial,saldoActual,estadoCredito) 
                Values(idCategoriaCredi,documentoUsuar,fechaInic,saldoInic,saldoActu,estadoCredi);
...
ORDER BY (c.documentoUsuario, c.idCategoriaCredito, c.idCredito);
...
(1, 2, 'Vinilo Aguamarina', 30, 1000, 1200),
(2, 1, 'Vinilo Dorado', 30, 1200, 1400),
(3, 2, 'Vinilo Plateado', 30, 1200, 1400);
...
--
-- Volcado de datos para la tabla `tblcategoriacredito`
--

INSERT INTO `tblcategoriacredito` (`idCategoriaCredito`, `nombreCategoria`) VALUES
(1, 'Clase'),
(2, 'Materiales');
...
('8101926', '1984-01-06', 'David', 'Cano Arango', 'dcanoara@gmail.com', '123', 1, NULL, 1, NULL),
